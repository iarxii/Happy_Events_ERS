  ...
  if ($entryPointVal == "pageinit") {
    ...
    try {
      ...
      if($rows<=0) {
          ...
          echo `<tr><td colspan="13"><h1 class="fw-bold text-center my-4">No Client User Accounts to Display!</h1></td></tr>`;
      } else {
        for ($j = 0; $j < $rows ; ++$j) {
          ...
          $comp_clients_list .= '<tr>
              <td class="text-center table-info">
                <button class="btn btn-info p-3 rounded-pill mb-2 shadow" onclick="showModal('."'$client_id'".','."'clients'".')" style="font-size: 30px"><i class="fas fa-edit"></i></button>
                <span style="font-size: 10px">Edit</span>
              </td>
              <td class="text-center table-danger">
                <button class="btn btn-danger p-3 rounded-pill mb-2 shadow" onclick="deleteClientRecord('."'$client_id'".')" style="font-size: 30px"><i class="fas fa-trash-alt"></i></button>
                <span style="font-size: 10px">Delete</span>
              </td>

              <td scope="row" class="fw-bold">'.$client_id.'</td>
              <td>'.$username.'</td>
              <td>'.$firstname.'</td>
              <td>'.$lastname.'</td>
              <td>'.$contact.'</td>
